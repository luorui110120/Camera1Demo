...
public class VideoToFrames implements Runnable {
    ...
    public static final int COLOR_FormatJPEG = 3;
    ...
    public interface Callback {
        ...
        void onDecodeFrame(int index, String filepath);
    }
    ...
    public void setSaveFrames(String dir,  int outType) throws IOException {
        mOutType = outType;
                    ...
                    if (mOutType != -1) {
                        ...
                        switch (mOutType) {
                            case COLOR_FormatI420:
                                ...
                                byte[] bytesI420 = getDataFromImage(image, COLOR_FormatI420);
                                Log.e("mlog","fileName:" + fileName + " + bytesI420 +  " + bytesI420.length);
                                dumpFile(fileName, bytesI420);
                                if (callback != null) {
                                    callback.onDecodeFrame(outputFrameCount, fileName);
                                }
                                break;
                            case COLOR_FormatNV21:
                                fileName = OUTPUT_DIR + String.format("frame_%05d_NV21_%dx%d.yuv", outputFrameCount, width, height);
                                byte[] bytesNV21 = getDataFromImage(image, COLOR_FormatNV21);
                                dumpFile(fileName, bytesNV21);
                                Log.e("mlog","fileName:" + fileName + " + bytesNV21 +  " + bytesNV21.length);
                                if (callback != null) {
                                    callback.onDecodeFrame(outputFrameCount, fileName);
                                }
                                break;
                            case COLOR_FormatJPEG:
